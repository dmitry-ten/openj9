...
TR_ResolvedMethodKey
   {
   TR_ResolvedMethodType _type;
   int32_t _cpIndex;
   TR_OpaqueClassBlock *_classObject; // only set for resolved interface methods

   bool operator==(const TR_ResolvedMethodKey &other) const
      {
      return _type == other._type && _cpIndex == other._cpIndex && _classObject == other._classObject;
      }
   };
...
namespace std
   ...
   struct hash<TR_ResolvedMethodKey>
      ...
      std::size_t operator()(const TR_ResolvedMethodKey &k) const
         ...
         return std::hash<int32_t>()(static_cast<int32_t>(k._type)) ^ std::hash<int32_t>()(k._cpIndex) ^ std::hash<TR_OpaqueClassBlock *>()(k._classObject);
         }
      };
   }
