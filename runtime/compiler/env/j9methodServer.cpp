...
TR_ResolvedJ9JITaaSServerMethod::getClassFromConstantPool(TR::Compilation * comp, uint32_t cpIndex, bool returnClassForAOT)
   ...
   TR_OpaqueClassBlock *ramClass = (TR_OpaqueClassBlock *) _ramClass;
   if (compInfoPT->getClientData()->getCachedClassFromConstantPool(_ramClass, cpIndex, resolvedClass))
      return resolvedClass;

   _stream->write(JITServer::MessageType::ResolvedMethod_getClassFromConstantPool, _remoteMirror, cpIndex, returnClassForAOT);
   resolvedClass = std::get<0>(_stream->read<TR_OpaqueClassBlock *>());
   if (resolvedClass)
      {
      compInfoPT->getClientData()->cacheClassFromConstantPool(_ramClass, cpIndex, resolvedClass);
      }
   return resolvedClass;
   }
