...
TR_J9ServerVM::getKnotIndexOfInvokeCacheArrayAppendixIndex(TR::Compilation *comp, uintptr_t *invokeCacheArray)
   {
   ...
   JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
   stream->write(
      JITServer::MessageType::VM_getKnotIndexOfInvokeCacheArrayAppendixIndex,
      invokeCacheArray
      );
...
TR_ResolvedMethod *
TR_J9ServerVM::targetResolvedMethodFromInvokeCacheArray(TR::Compilation *comp, TR_ResolvedMethod *owningMethod, uintptr_t *invokeCacheArray)
   {
   TR_OpaqueMethodBlock *targetMethodObj = 0;
   JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
   stream->write(
      JITServer::MessageType::VM_targetResolvedMethodFromInvokeCacheArray,
...
TR::SymbolReference*
TR_J9ServerVM::refineInvokeCacheElementSymRefWithKnownObjectIndex(TR::Compilation *comp, TR::SymbolReference *originalSymRef, uintptr_t *invokeCacheArray, int32_t arrayIndex)
   {
   TR::KnownObjectTable *knot = comp->getOrCreateKnownObjectTable();
...
TR_J9ServerVM::jniMethodIdFromMemberName(TR::Compilation* comp, TR::KnownObjectTable::Index objIndex)
   ...
   if (objIndex != TR::KnownObjectTable::UNKNOWN &&
      ...
      {
      JITServer::ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
      stream->write(JITServer::MessageType::VM_jniMethodIdFromMemberName, objIndex);
      return std::get<0>(stream->read<J9JNIMethodID *>());
      }
   return NULL;
   }
